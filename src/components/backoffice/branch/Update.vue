<template>
  <div class="container-fluid">
    <form-header title="Update Branch" />
...
export default {
  ...
  methods: {
    submit(event) {
      ...
      BranchService.postUpdate(Utils.decrypt(this.$route.query.id), this.branch).then((response) => {
    ...
    getBranch() {
      let loading = this.$loading.show();
      BranchService.getShow(Utils.decrypt(this.$route.query.id)).then(
        (response) => {
          if (response.code === 200) {
