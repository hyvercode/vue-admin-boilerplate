    ...
    >
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form role="form">
            <div class="mb-3">
              <label class="form-label" for="menuName">Category Name</label>
              <input type="text" class="form-control" v-model="category.category_name" placeholder="Please input"
                     required>
            </div>
            <div class="mb-3">
              <label class="form-label" for="href">Category Slug</label>
              <input type="text" class="form-control" v-model="category.category_slug" placeholder="Please input"
                     required>
            </div>
            <div class="mb-3">
              <label class="form-label" for="href">Category Descriptions</label>
              <textarea cols="5" rows="5" class="form-control" v-model="category.descriptions"
...
import BlogCategoriesService from "../../../services/blogCategory.service";
import RequestBlogCategory from "../../../payloads/request/RequestBlogCategory";
...
export default {
  ...
  data() {
    return {
      category: new RequestBlogCategory(),
      columns: [
        {
          label: "ID",
          field: "id",
      ...
      sort: 'DESC',
      filterRecord: [
        {'id': 'id', "desc": "ID Category"},
        {'id': 'category_name', "desc": "Category Name"},
        {'id': 'category_slug', "desc": "Category Slug"},
  ...
  methods: {
    ...
    getRecordPaginate(dateFrom, dateTo, searchBy, searchParam, perPage, currentPage, sortBy, sort) {
      ...
      BlogCategoriesService.getPaginate(payload).then((response) => {
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          BlogCategoriesService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "top-end",
                  toast: true,
    ...
    getShow() {
      let loading = this.$loading.show();
      BlogCategoriesService.getShow(this.category.id).then((response) => {
        if (response.code === 200) {
    ...
    handleSubmit() {
      let loading = this.$loading.show();
      if (!this.idUpdate) {
        BlogCategoriesService.postCreate(this.category).then((response) => {
          if (response.code === 200) {
            this.category= new RequestBlogCategory();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
        ...
        BlogCategoriesService.postUpdate(this.category.id, this.category).then((response) => {
          if (response.code === 200) {
            this.category = new RequestBlogCategory();
            loading.hide();
            this.$swal.fire({
