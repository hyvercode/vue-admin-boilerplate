...
export default {
  ...
  methods: {
    ...
    getRecordPaginate(dateFrom, dateTo, searchBy, searchParam, perPage, currentPage, sortBy, sort) {
      ...
      BlogCommentService.getPaginate(payload).then((response) => {
    ...
    doCheckToggle(props) {
      let payload = props;
      payload.comment_approved = !props.comment_approved
      let loading = this.$loading.show();
      BlogCommentService.postUpdate(props.id, payload).then((response) => {
        if (response.code === 200) {
          this.doRefresh();
          loading.hide();
        } else {
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          BlogCommentService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "top-end",
                  toast: true,
