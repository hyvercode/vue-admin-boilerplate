          ...
          <div class="card px-3 py-3 mb-3">
            ...
            <div class="row text-right mt-3">
              <div class="col-lg-12 d-flex justify-content-end">
                ...
                <button class="btn btn-primary" v-if="post.published" style="margin-left: 10px !important"
                        @click.prevent="handleDraft">
                  {{ post.published ? 'Draft' : 'Publish' }}
                </button>
                <button v-else class="btn btn-primary" style="margin-left: 10px !important"
...
export default {
  ...
  methods: {
    submit(event) {
      ...
      BlogPostService.postCreate(this.post).then((response) => {
        if (response.code === 200) {
          this.post = new RequestBlogPost();
          loading.hide();
          this.$swal.fire({
            icon: "success",
            title: "Success",
