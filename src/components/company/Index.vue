...
export default {
  ...
  methods: {
    ...
    handleCreate() {
      router.push(Pages.COMPANY_CREATE);
    },
    async handleUpdate(prop) {
      await router.push(Pages.COMPANY_UPDATE.concat(Utils.encrypt(prop.id.toString())));
    },
    handleDelete(prop) {
      console.log(prop.id);
    },
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      CompaniesService.getPaginate(params).then(response => {
