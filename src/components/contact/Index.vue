<template>
  <div class="container-fluid">
    ...
    <b-modal id="m-jobtitle" :title="isUpdate?'Update Contact':'Create Contact'" hide-footer>
      <form @submit.prevent="submit($event)">
        ...
        <div class="form-group mb-3">
          <label>Fax Number <span class="mandatory">*</span></label>
          <input
              type="number"
              class="form-control"
              placeholder="Please input text"
        ...
        <div class="col-lg-12 mb-3">
          ...
          <select class="form-control form-select" v-model="contact.active" required>
...
import RequestContact from "@/payloads/request/RequestContact";
...
export default {
  ...
  data() {
    return {
      ...
      records: [],
      filterRecord: [
        {id: 'id', desc: "Contact ID"},
        {id: 'contact_name', desc: "Contact Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        ContactService.postCreate(this.contact).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestContact();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
        ...
        ContactService.postUpdate(this.contact.id, this.contact).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestContact();
            loading.hide();
            this.$swal.fire({
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      ContactService.getPaginate(params).then((response) => {
        if (response.code === 200) {
