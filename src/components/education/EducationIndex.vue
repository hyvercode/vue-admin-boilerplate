          ...
          <form>
            ...
            <div class="mb-3">
              <div class="form-check">
                <input class="form-check-input" type="checkbox" v-model="education.active" id="active" required>
...
export default {
  ...
  data() {
    return {
      education: new RequestEducation(),
      columns: [
        {
          label: "ID",
          field: "id",
      ...
      sort: 'DESC',
      filterRecord: [
        {'id': 'id', "desc": "ID Education"},
        {'id': 'education_name', "desc": "Description"}
      ],
      pagination: {
  ...
  methods: {
    getRecordPaginate(dateFrom, dateTo, searchBy, searchParam, perPage, currentPage, sortBy, sort) {
      ...
      EducationsService.getPaginate(payload).then((response) => {
    ...
    doCheckToggle(props) {
      ...
      let loading = this.$loading.show();
      EducationsService.postUpdate(props.id, payload).then((response) => {
        if (response.code === 200) {
          this.doRefresh();
          loading.hide();
        } else {
    ...
    getShow() {
      let loading = this.$loading.show();
      EducationsService.getShow(this.education.id).then((response) => {
        if (response.code === 200) {
    ...
    handleSubmit() {
      let loading = this.$loading.show();
      if (!this.idUpdate) {
        EducationsService.postCreate(this.education).then((response) => {
          if (response.code === 200) {
            this.education = new RequestEducation();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
        ...
        EducationsService.postUpdate(this.education.id, this.education).then((response) => {
          if (response.code === 200) {
            this.education = new RequestEducation();
            loading.hide();
            this.$swal.fire({
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          // <-- if confirmed
          EducationsService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "top-end",
                  toast: true,
