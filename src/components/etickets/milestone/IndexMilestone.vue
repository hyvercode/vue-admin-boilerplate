<template>
  <div class="container-fluid">
    ...
    <b-modal id="m-jobposition" :title="isUpdate?'Update E-Ticket Milestone':'Create E-Ticket Milestone'" hide-footer>
      <form @submit.prevent="submit($event)">
        <div class="form-group mb-3">
          <label>Milestone Name <span class="mandatory">*</span></label>
          <input
              type="text"
              class="form-control"
              placeholder="Please input text"
              v-model="eticketmilestone.milestone_name"
              required
          />
        </div>
        <div class="col-lg-12 mb-3">
          <label for="NIK" style="text-align: left">Active <span class="mandatory">*</span></label>
          <select class="form-control form-select" v-model="eticketmilestone.active" required>
...
import RequestEticketMilestone from "@/payloads/request/RequestEticketMilestone";
...
export default {
  ...
  data() {
    return {
      eticketmilestone: new RequestEticketMilestone(),
      searchBy: "id",
      searchParam: "",
      dateFrom: "",
      dateTo: "",
      ...
      records: [],
      filterRecord: [
        {id: 'id', desc: "Milestone ID"},
        {id: 'milestone_name', desc: "Milestone Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        EticketmilestoneService.postCreate(this.eticketmilestone).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestEticketMilestone();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
              text: "ETicket Milestone has been created",
            });
            this.doRefresh();
            this.$bvModal.hide('m-jobposition');
          } else {
        ...
        EticketmilestoneService.postUpdate(this.eticketmilestone.id, this.eticketmilestone).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestEticketMilestone();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
              text: "ETicket Milestone has been updated",
            });
            this.doRefresh();
            this.$bvModal.hide('m-jobposition');
          } else {
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          // <-- if confirmed
          EticketmilestoneService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "bottom-end",
                  toast: true,
    ...
    doCheckToggle(props) {
      ...
      let loading = this.$loading.show();
      EticketmilestoneService.postUpdate(props.id, payload).then((response) => {
        if (response.code === 200) {
          this.doRefresh();
          loading.hide();
        } else {
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      EticketmilestoneService.getPaginate(params).then((response) => {
        if (response.code === 200) {
