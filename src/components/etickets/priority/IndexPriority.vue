<template>
  <div class="container-fluid">
    ...
    <b-modal id="m-jobposition" :title="isUpdate?'Update E-Ticket Priority':'Create E-Ticket Priority'" hide-footer>
      <form @submit.prevent="submit($event)">
        <div class="form-group mb-3">
          <label>Priority Name <span class="mandatory">*</span></label>
          <input
              type="text"
              class="form-control"
              placeholder="Please input text"
              v-model="eticketpriority.priority_name"
              required
          />
        </div>
        <div class="col-lg-12 mb-3">
          <label for="NIK" style="text-align: left">Active <span class="mandatory">*</span></label>
          <select class="form-control form-select" v-model="eticketpriority.active" required>
...
export default {
  ...
  data() {
    return {
      eticketpriority: new RequestEticketPriority(),
      searchBy: "id",
      searchParam: "",
      dateFrom: "",
      dateTo: "",
      ...
      records: [],
      filterRecord: [
        {id: 'id', desc: "Priority ID"},
        {id: 'priority_name', desc: "Priority Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        ETicketPriorityService.postCreate(this.eticketpriority).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestEticketPriority();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
              text: "ETicket Priority has been created",
            });
            this.doRefresh();
            this.$bvModal.hide('m-jobposition');
          } else {
        ...
        ETicketPriorityService.postUpdate(this.eticketpriority.id, this.eticketpriority).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestEticketPriority();
            loading.hide();
            this.$swal.fire({
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          // <-- if confirmed
          ETicketPriorityService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "bottom-end",
                  toast: true,
    ...
    doCheckToggle(props) {
      ...
      let loading = this.$loading.show();
      ETicketPriorityService.postUpdate(props.id, payload).then((response) => {
        if (response.code === 200) {
          this.doRefresh();
          loading.hide();
        } else {
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      ETicketPriorityService.getPaginate(params).then((response) => {
        if (response.code === 200) {
