<template>
  <div class="container-fluid">
    ...
    <b-modal id="m-bsc" :title="isUpdate?'Update KPI Group':'Create KPI Group'" hide-footer>
      <form @submit.prevent="submit($event)">
        <div class="form-group mt-1">
          ...
          <input
              ...
              v-model="kpigroup.kpi_group_name"
              required
          />
        </div>
        <div class="col-lg-12 mb-3">
          <label for="status" style="text-align: left">Active</label>
          <select class="form-control form-select" v-model="kpigroup.active" required>
...
export default {
  ...
  data() {
    return {
      ...
      records: [],
      filterRecord: [
        {id: 'id', desc: "KPI Group ID"},
        {id: 'kpi_group_name', desc: "KPI Group Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        KpiGroupService.postCreate(this.kpigroup).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestKpiGroup();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
        ...
        KpiGroupService.postUpdate(this.kpigroup.id, this.kpigroup).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestKpiGroup();
            loading.hide();
            this.$swal.fire({
    ...
    handleCreate() {
      this.kpigroup = new RequestKpiGroup();
      this.isUpdate = false;
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          // <-- if confirmed
          KpiGroupService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "bottom-end",
                  toast: true,
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      KpiGroupService.getPaginate(params).then((response) => {
        if (response.code === 200) {
