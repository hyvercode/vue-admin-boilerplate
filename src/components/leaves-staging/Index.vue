<template>
  <div class="container-fluid">
    ...
    <b-modal
        id="m-leave-staging"
        :title="isUpdate ? 'Update Staging' : 'Create Staging '"
        hide-footer
    >
      <form @submit.prevent="submit($event)">
        <div class="col-lg-12 mb-3">
          <label for="unit" style="text-align: left"
          >Unit <span class="mandatory">*</span></label
          >
          <select
              class="form-control form-select"
              v-model="leavestaging.unit_id"
              id="unit"
              required
          >
        ...
        <div class="col-lg-12 mb-3">
          <label for="targetType" style="text-align: left"
          >Reviewer<span class="mandatory">*</span></label
          >
          <select
              class="form-control form-select"
              v-model="leavestaging.reviewer_id"
              id="targetType"
              required
          >
            <option value="null" disabled>Choose...</option>
        ...
        <div class="col-lg-12 mb-3">
          <label for="targetType" style="text-align: left"
          >Approver <span class="mandatory">*</span></label
          >
          <select
              class="form-control form-select"
              v-model="leavestaging.approver_id"
              id="targetType"
              required
          >
            <option value="null" disabled>Choose...</option>
...
export default {
  ...
  data() {
    return {
      leavestaging: new RequestLeaveStaging(),
      searchBy: "id",
      searchParam: "",
      dateFrom: "",
      dateTo: "",
  ...
  methods: {
    ...
    doClose() {
      this.$bvModal.hide("m-leave-staging");
      this.doRefresh();
    },
    handleCreate() {
      this.isUpdate = false;
      this.leavestaging = new RequestLeaveStaging();
      this.$bvModal.show("m-leave-staging");
    },
    handleUpdate(prop) {
      this.isUpdate = true;
      this.leavestaging = prop;
      this.$bvModal.show("m-leave-staging");
    },
    submit(event) {
      ...
      if (!this.isUpdate) {
        LeaveStagingService.postCreate(this.leavestaging).then((response) => {
        ...
        LeaveStagingService.postUpdate(
            this.leavestaging.id,
            this.leavestaging
        ).then((response) => {
          if (response.code === 200) {
            this.leavestaging = new RequestLeaveStaging();
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          LeaveStagingService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "bottom-end",
                  toast: true,
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      LeaveStagingService.getPaginate(params).then((response) => {
        if (response.code === 200) {
