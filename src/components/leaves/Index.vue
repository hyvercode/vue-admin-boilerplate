    ...
    >
      <form @submit.prevent="submit($event)">
        <div class="form-group mb-3">
          <label>Leave Name <span class="mandatory">*</span></label>
          <input
              type="text"
              class="form-control"
              placeholder="Please input text"
              v-model="Leaves.leave_name"
              required
          />
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      records: [],
      filterRecord: [
        {id: "id", desc: "Leaves ID"},
        {id: "leave_name", desc: "Leave Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        leavesService.postCreate(this.Leaves).then((response) => {
        ...
        leavesService
            .postUpdate(this.Leaves.id, this.Leaves)
            .then((response) => {
              if (response.code === 200) {
                this.Leaves = new RequestLeaves();
                loading.hide();
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      leavesService.getPaginate(params).then((response) => {
        if (response.code === 200) {
