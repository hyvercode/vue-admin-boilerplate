<template>
  <div class="container-fluid">
    ...
    <b-modal id="m-bsc" :title="isUpdate?'Update Target Type':'Create Target Type'" hide-footer>
      <form @submit.prevent="submit($event)">
        <div class="form-group mt-1 mb-3">
          ...
          <input
              ...
              v-model="targettype.target_type_name"
              required
          />
        </div>
        <div class="col-lg-12 mb-3">
          <label for="calculate" style="text-align: left">Calculate</label>
          <select class="form-control form-select" v-model="targettype.is_calculate" required>
            <option value="null" disabled>Choose...</option>
            <option
                v-for="item in calculated"
                :key="item.id"
                :value="item.id"
            >
              {{ item.desc }}
        ...
        <div class="col-lg-12 mb-3">
          ...
          <select class="form-control form-select" v-model="targettype.active" required>
            <option value="null" disabled>Choose...</option>
            <option
...
export default {
  ...
  data() {
    return {
      targettype: new RequestTargetType(),
      searchBy: "id",
      searchParam: "",
      dateFrom: "",
      dateTo: "",
      calculated: Utils.calculated(),
      status: Utils.status(),
      columns: [
        {
          label: "ID",
      ...
      records: [],
      filterRecord: [
        {id: 'id', desc: "Target Type ID"},
        {id: 'target_type_name', desc: "Target Type Name"},
  ...
  methods: {
    ...
    submit(event) {
      ...
      if (!this.isUpdate) {
        TargetTypeService.postCreate(this.targettype).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestTargetType();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
              text: "Kpi Group has been created",
            });
            this.doRefresh();
            this.$bvModal.hide('m-bsc');
          } else {
        ...
        TargetTypeService.postUpdate(this.targettype.id, this.targettype).then((response) => {
          if (response.code === 200) {
            this.coverage = new RequestTargetType();
            loading.hide();
            this.$swal.fire({
              icon: "success",
              title: "Success",
              text: "Kpi Group has been updated",
            });
            this.doRefresh();
            this.$bvModal.hide('m-bsc');
          } else {
    ...
    handleCreate() {
      this.targettype = new RequestTargetType();
      this.isUpdate = false;
    ...
    handleDelete(prop) {
      this.$swal({
        ...
        if (result.value) {
          // <-- if confirmed
          TargetTypeService.delete(prop.id)
              .then(() => {
                this.$swal({
                  position: "bottom-end",
                  toast: true,
    ...
    getPaginate(dateFrom, dateTo, searchBy, searchParam, limit, page) {
      ...
      let loading = this.$loading.show();
      TargetTypeService.getPaginate(params).then((response) => {
        if (response.code === 200) {
